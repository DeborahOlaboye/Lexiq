...
export default function GameBoard({ roundId, onBack }: { roundId: bigint | null; onBack: () => void }) {
  ...
  const isActive = timeLeft > 0 && phase === "active";
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-3">
        <div className="flex-1 bg-gray-900 rounded-xl p-3 text-center"><p className="text-xs text-gray-500">Score</p><p className="text-2xl font-bold text-violet-400">{myScore}</p></div>
      ...
      {address && <p className="text-[10px] text-center text-gray-700">{address.slice(0,6)}...{address.slice(-4)}</p>}
    </div>
  );
}
