        ...
        <div className="relative rounded-[22px] overflow-hidden" style={{ background: "#1E1710", border: LINE }}>
          ...
          <div className="relative flex flex-col items-center text-center" style={{ padding: "clamp(24px,5vw,44px)" }}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center", marginBottom: 16 }}>
              ...
              <div style={{ display: "inline-flex", padding: "7px 14px", borderRadius: 20, background: "rgba(207,233,75,.12)", border: "1px solid rgba(207,233,75,.3)", fontFamily: "var(--font-mono)", fontSize: 11, color: "#CFE94B" }}>
                {DIFF_LABEL[difficulty]} · {DURATIONS[difficulty]}s
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, width: "100%", marginTop: 20 }}>
              <motion.button whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.97 }} onClick={onBack}
                ...
                transition={{ duration: 2.2, repeat: Infinity, ease: "easeInOut" }}
                style={{ flex: 1, padding: "clamp(13px,2.5vw,16px)", borderRadius: 15, border: "none", background: "#CFE94B", color: "#15110D", fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(15px,2vw,17px)", boxShadow: "0 5px 0 #A9C931", cursor: "pointer" }}>
                Play again
              </motion.button>
              {onLeaderboard && (
                <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={onLeaderboard}
                  style={{ flex: 1, padding: "clamp(13px,2.5vw,16px)", borderRadius: 15, background: "none", border: LINE2, color: "#F5EFE2", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "clamp(15px,2vw,17px)", cursor: "pointer" }}>
                  Rankings
                </motion.button>
              )}
            </div>
            ...
            <div style={{ position: "relative", marginBottom: 12 }}>
              <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
                <input value={input}
                  ...
                  style={{ flex: 1, minWidth: 0, background: "#1E1710", borderRadius: 13, padding: "clamp(11px,2vw,14px) clamp(12px,2vw,16px)", fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(16px,3vw,18px)", letterSpacing: "0.14em", color: "#F5EFE2", textTransform: "uppercase", outline: "none",
                    border: wordValid === "valid" ? "1px solid #CFE94B" : wordValid === "invalid" ? "1px solid rgba(255,91,69,.6)" : LINE2 }} />
